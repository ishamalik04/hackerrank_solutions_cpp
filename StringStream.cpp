...
vector<int> parseInts(string str) {
   ...
   stringstream ss(str);//Let us write and endless stream of strings
    vector<int> result;
    char remove;
    int tmp;
    while(ss >> tmp){//Extracted integer one at a time
        result.push_back(tmp);
        ss >> remove;//used to remove the comma in the input
    }
    return result;
}
